...
const mockUserRepository = {
  findById: jest.fn(),
  userModel: {
    /* Mock any needed model properties if necessary */
  },
};
...
describe("TherapistService", () => {
  ...
  describe("createTherapist", () => {
    it("should create a therapist profile successfully", async () => {
      ...
      expect(mockTherapistRepository.create).toHaveBeenCalledWith(
        therapistData,
      );
    ...
    it("should throw a 409 error if the user is already a therapist", async () => {
      const mockUser = { id: 1, username: "testuser" };
      mockUserRepository.findById.mockResolvedValue(mockUser);
      mockTherapistRepository.findByUserId.mockResolvedValue({
        id: 10,
        userId: 1,
      });
      ...
      ).rejects.toThrow("This user is already registered as a therapist.");
    });
  });
  ...
  describe("getTherapistById", () => {
    it("should return a therapist profile with user data", async () => {
      ...
      expect(mockTherapistRepository.findById).toHaveBeenCalledWith(
        10,
        expect.any(Object),
      );
  ...
  describe("deleteTherapist", () => {
    ...
    it("should throw a 404 error if the therapist profile to delete is not found", async () => {
      ...
      await expect(therapistService.deleteTherapist(99)).rejects.toThrow(
